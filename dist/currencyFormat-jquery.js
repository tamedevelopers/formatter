 ...
 * https://github.com/tamedevelopers/components
...
(function($) {
    ...
    const __tameCurrencyFormat = function(options) {
        var countryCurrency = __tameGetCountryCurrencies();

        // check if currency symbol is passed
        function __tameCheckCurrencySymbol(string){
        ...
        // get format type
        function __tameGetStyleFormat(style){
            if (style.trim().toLowerCase() === 'currency' || style.trim().toLowerCase() === 'decimal') {
                return style;
        ...
        function __tameGetCurrencySymbol(options){
        ...
        var intNumber = new Intl.NumberFormat(settings.language, {
        ...
        // get amount as float alone 
        settings.amount = parseFloat(settings.amount);

        // format amount
        let formattedNumber = intNumber.format(settings.amount);

        // get an array of formatted parts of the amount
        let matches = intNumber.formatToParts(settings.amount);

        // only try if currency display is allowed
        if(settings.display){
            // Extract currency symbol
            matches = intNumber.formatToParts(settings.amount).find(function(part) {
    ...
    $.fn.tameCurrency = function(options = {}){
        $(this).text(  __tameCurrencyFormat(options)  );
        return;
    };
